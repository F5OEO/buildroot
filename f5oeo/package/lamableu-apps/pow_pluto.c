...
//	printf("* Configuring receiver at %lli Hz (lo), samplerate %ld MHz (fs) \n",rxcfg.lo_hz,fs_mhz);
  ...
  struct iio_scan_context *sctx = iio_create_scan_context("local",0);
//struct iio_scan_context *sctx = iio_create_context_from_uri("local");
    if (sctx==NULL) {
...
//    printf("ret %d\n", ret);
    ...
    if (ret == 0) {
        ...
        iio_scan_context_destroy(sctx);
        fprintf(stderr, "Noo Pluto device found\n");
    }
